...
test('EXPLOSION installs in four seconds, shows a two-second blast before the result, and round 10 ends the match',()=>{const a=arena('EXPLOSION'),site=a.bombSites[0];a.entities.forEach(e=>{if(e.team==='RED'){e.x=site.x;e.y=site.y;}else{e.x=a.map.spawns.BLUE[0].x;e.y=a.map.spawns.BLUE[0].y;}});a.updateExplosion(4);assert.equal(a.bomb.planted,true);assert.equal(a.bomb.site,'A');a.time=a.bomb.explodeAt;a.updateExplosion(.01);assert.equal(a.bomb.detonating,true);assert.equal(a.result,null);assert.ok(a.effects.some(e=>e.type==='bomb-explosion'&&e.maxLife===2));advance(a,1.9);assert.equal(a.state,'PLAYING');assert.equal(a.result,null);advance(a,.15);assert.equal(a.result.winner,'RED');const b=arena('EXPLOSION');b.round=10;b.score={BLUE:4,RED:5};b.finishRound('BLUE','test');assert.equal(b.result.match,true);assert.equal(b.result.draw,true);});
test('EXPLOSION keeps a planted bomb alive after attackers are wiped, but defender wipe awards attackers immediately',()=>{const a=arena('EXPLOSION');a.bomb.planted=true;a.bomb.x=a.bombSites[0].x;a.bomb.y=a.bombSites[0].y;a.bomb.explodeAt=a.time+20;for(const e of a.entities)if(e.team===a.attackTeam)e.alive=false;a.checkRules();assert.equal(a.state,'PLAYING');assert.equal(a.result,null);for(const e of a.entities)if(e.team===a.defendTeam)e.alive=false;a.checkRules();assert.equal(a.state,'ROUND_END');assert.equal(a.result.winner,a.attackTeam);});
test('AI EXPLOSION randomly assigns the player to the attacking or defending starting side',()=>{const original=Math.random;try{Math.random=()=>.1;const attack=arena('EXPLOSION');assert.equal(attack.player().team,'RED');assert.equal(attack.player().team,attack.attackTeam);assert.equal(attack.entities.filter(e=>e.team==='RED').length,5);assert.equal(attack.entities.filter(e=>e.team==='BLUE').length,5);Math.random=()=>.9;const defend=arena('EXPLOSION');assert.equal(defend.player().team,'BLUE');assert.equal(defend.player().team,defend.defendTeam);assert.equal(defend.entities.filter(e=>e.team==='RED').length,5);assert.equal(defend.entities.filter(e=>e.team==='BLUE').length,5);}finally{Math.random=original;}});
...
test('online EXPLOSION uses the planted-bomb elimination rule, INFINITY uses kills, BOSS uses actual assigned boss',async()=>{
 ...
 n=onlineFixture('EXPLOSION');n.shared.bomb={...n.shared.bomb,planted:true,site:'A',x:320,y:320,explodeAt:n.now()+10000};for(const [id,s]of Object.entries(n.states))if(n.roster[id].team==='RED')s.alive=false;n.hostTick();await Promise.resolve();assert.equal(n.shared.phase,'PLAYING');assert.equal(n.shared.result,undefined);n.setClock(n.now()+200);for(const [id,s]of Object.entries(n.states))if(n.roster[id].team==='BLUE')s.alive=false;n.hostTick();await Promise.resolve();assert.equal(n.shared.result.winner,'RED');
...
test('online EXPLOSION keeps PLAYING for the two-second detonation presentation before publishing the result',async()=>{const n=onlineFixture('EXPLOSION');n.shared.bomb={...n.shared.bomb,planted:true,site:'A',x:320,y:320,explodeAt:n.now()};n.hostTick();await Promise.resolve();assert.equal(n.shared.phase,'PLAYING');assert.equal(n.shared.bomb.detonating,true);assert.equal(n.shared.result,undefined);n.applySharedState();assert.ok(n.arena.effects.some(e=>e.type==='bomb-explosion'));n.setClock(n.now()+1900);n.hostTick();await Promise.resolve();assert.equal(n.shared.phase,'PLAYING');n.setClock(n.now()+200);n.hostTick();await Promise.resolve();assert.equal(n.shared.phase,'ROUND_END');assert.equal(n.shared.result.winner,'RED');});
