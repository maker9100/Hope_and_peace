...
(function(){
  ...
  const config=storage.get('firebase');if(config)dom['firebase-config'].value=config;
  ...
  dom['ai-menu'].onclick=()=>{nickname();setPanel('modes');audio.unlock();};dom['online-menu'].onclick=()=>{nickname();setPanel('online');audio.unlock();if(!net.hasConfig()){dom['online-status'].textContent='Firebase 설정을 연결하면 온라인 방을 만들 수 있다. AI 전투는 바로 실행된다.';dom['firebase-settings'].open=true;}updateRoom();};
  ...
  for(const d of [dom['score-dialog'],dom['controls-dialog']])d.addEventListener('click',e=>{if(e.target===d){const b=d.getBoundingClientRect();if(e.clientX<b.left||e.clientX>b.right||e.clientY<b.top||e.clientY>b.bottom)d.close();}});
  ...
  dom['save-config'].onclick=()=>{try{if(net.code)throw new Error('방에서 나온 뒤 설정을 변경한다.');const cfg=C.FirebaseArena.validateConfig(JSON.parse(dom['firebase-config'].value));storage.set('firebase',JSON.stringify(cfg));window.CHEAT_ARENA_FIREBASE=cfg;net.resetConnection();notify('Firebase 공개 설정을 저장했다.');dom['online-status'].textContent='설정 저장됨 · 방을 만들거나 참가할 수 있다.';dom['firebase-settings'].open=false;}catch(e){dom['online-status'].textContent=e.message;}};
  function updateRoom(){
    const inside=!!net.code;dom['online-connect'].hidden=inside;dom.room.hidden=!inside;if(!inside)return;
  ...
  window.addEventListener('keydown',e=>{
    ...
    const key=e.key.toLowerCase();if(['f1','f2','f3',' ','control','tab','w','a','s','d','c','r'].includes(key))e.preventDefault();if(dom['score-dialog'].open||dom['controls-dialog'].open){if(key==='tab')dom['score-dialog'].close();return;}
  ...
  function updateHUD(now){
    ...
    dom['weapon-name'].textContent=w.label;dom['ammo-value'].textContent=view.weapon===2?'∞':view.ammo[view.weapon];dom['ammo-reserve'].textContent=view.weapon===2?'MELEE':'/ ∞';dom['reload-label'].textContent=view.reloadLeft>0?`RELOADING · ${view.reloadLeft.toFixed(1)}s`:w.auto?'FULL AUTO':view.weapon===2?'CLOSE QUARTERS':'SEMI AUTO';dom['reload-track'].firstElementChild.style.width=view.reloadLeft>0?((1-view.reloadLeft/view.reloadTotal)*100)+'%':'0%';
